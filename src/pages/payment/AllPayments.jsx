...
const AllPayments = () => {
    ...
    useEffect(()=>{
        const fetchData=()=>{
            fetch('http://localhost:3010/payment/allpayment')
    ...
    const handleFilter=(e)=>{
        if(e.target.value==''){
            ...
            const filterData=searchApi.filter(item=>item.type.toLowerCase().includes(e.target.value.toLowerCase()));
            setUsers(filterData);
        }
    ...
    return (
        <>
            ...
            <Button className="link" startIcon={<AddIcon />} variant="outlined" component={Link} to="/addpayment" >Add New</Button>
            ...
            <PaymentsList users={currentPosts} updateUsers={setUsers} />
            <PaymentsPagination
